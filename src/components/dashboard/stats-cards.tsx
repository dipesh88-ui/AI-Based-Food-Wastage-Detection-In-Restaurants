...
export default function StatsCards({wasteData}: StatsCardsProps) {
  ...
  const stats = useMemo(() => {
    ...
    // Aggregation by individual items
    ...
    dataToFilter.forEach(entry => {
      if (entry.analysisDetails?.items && entry.analysisDetails.items.length > 0) {
        entry.analysisDetails.items.forEach(item => {
          ...
          itemQuantities[name] = (itemQuantities[name] || 0) + grams;
          totalGrams += grams;
        });
      } else {
        ...
        itemQuantities[fallbackType] = (itemQuantities[fallbackType] || 0) + quantity;
        totalGrams += quantity;
      }
    });
    ...
    const mostWastedFoodType =
      Object.keys(itemQuantities).length > 0
        ? Object.entries(itemQuantities).sort((a, b) => b[1] - a[1])[0][0]
        : 'N/A';

    return {totalEntries, totalQuantity: totalGrams, mostWastedFoodType};
  }, [wasteData, period]);
